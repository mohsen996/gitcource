...
class AccountReportGeneralLedger(models.TransientModel):
    ...
    show_credit = fields.Boolean(string="Credit?")
    show_debit = fields.Boolean(string="Debit?")
    def _print_report(self, data):
        data = self.pre_print_report(data)
        data['form'].update(self.read(['initial_balance', 'sortby','show_credit','show_debit'])[0])
